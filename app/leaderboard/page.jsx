  ...
  useEffect(() => {
    const fetchRewardsAndUser = async () => {
      ...
      try {
        ...
        for (const reward of fetchedRewards) {
          if (rewardMap.has(reward.userId)) {
            const existing = rewardMap.get(reward.userId)
            rewardMap.set(reward.userId, {
              ...existing,
              points: existing.points + reward.points,
            })
          } else {
            rewardMap.set(reward.userId, { ...reward })
          }
        }

        // Convert map to array and sort descending by points
        const mergedRewards = Array.from(rewardMap.values()).sort(
