...
export default function RewardsPage() {
      ...
      try {
        ...
        if (userEmail) {
          ...
          if (fetchedUser) {
            ...
            setRewards(fetchedRewards.filter(r => Number(r.cost) > 0)) // Fix: ensure numeric comparison
  ...
  const handleRedeemReward = async (rewardId) => {
    ...
    if (reward && Number(reward.cost) > 0 && balance >= reward.cost) {
      try {
  ...
  const refreshUserData = async () => {
    if (user) {
      ...
      if (fetchedUser) {
        ...
        setTransactions(fetchedTransactions)
        const fetchedRewards = await getAvailableRewards(fetchedUser.id)
        setRewards(fetchedRewards.filter(r => Number(r.cost) > 0)) // Fix: consistent filter
              ...
              rewards.map(reward => (
                <div key={reward.id} className="bg-white p-4 rounded-xl shadow-md">
                  ...
                  <Button
                    ...
                    className="w-full bg-green-500 hover:bg-green-600 text-white"
                    disabled={balance < reward.cost || Number(reward.cost) <= 0}
                  >
                    <Gift className="w-4 h-4 mr-2" />
                    Redeem Reward
                  </Button>
